    ...
    public JSONObject authenticate(AuthenticationRequest request) {
        ...
        if (user.getUserKind() == UserKind.SHOP)            //User Shop
        {
            if (user.getRole() == ADMIN) {
                ...
                infList=new String[]{"شماره فاکتور@شناسه فروشنده/توضیحات","شماره فاکتور","تعداد کالا@تاریخ","مبلغ(ریال)@چک"};
                checkBoxesList=new String[]{"chk1","chk2","chk3"};
                panels=new String[][]{{"نحوه ارسال","نحوه دریافت","دستور","نوع"},{"صدا","تصویر","صداوتصویر","هیچکدام"},{"صدا","تصویر","صداوتصویر","هیچکدام"},{"بدهی","طلب","گزارش کالا","فاکتور"},{"ما","ایشان","صورتحساب فاکتور"}};
