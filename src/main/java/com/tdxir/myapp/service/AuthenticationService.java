    ...
    public JSONObject authenticate(AuthenticationRequest request) {
        ...
        if (user.getUserKind() == UserKind.SHOP)            //User Shop
        {
            if (user.getRole() == ADMIN) {
                ...
                infList=new String[]{"شماره فاکتور@شناسه فروشنده","شماره فاکتور","تعداد کالا@تاریخ فاکتور","قیمت(ریال)"};
                checkBoxesList=new String[]{};//"chk1","chk2","chk3"};
                panels=new String[][]{{"نحوه ارسال","نحوه دریافت","دستور"},{"صدا","تصویر","صداوتصویر","هیچکدام"},{"صدا","تصویر","صداوتصویر","هیچکدام"},{"ثبت فاکتور","ثبت چک","گزارش کالا","گزارش فاکتور"}};
